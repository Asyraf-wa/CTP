...
?>
		...
		<div class="col-md-4">
			...
			<?php echo $this->Html->image('../files/articles/poster/' . $article->poster_dir . '/' . $article->poster,['class' =>'article-header-cover', 'width' => '500px', 'height' => '500px']); ?>
...
?>
<hr>
<?= $this->Html->link(__('<i class="far fa-edit"></i> Edit'), ['action' => 'edit', $article->id], ['class' => 'btn btn-secondary btn-sm mt-1', 'escape' => false]) ?> 
<?= $this->Form->postLink(__('<i class="far fa-trash-alt"></i> Delete'), ['action' => 'delete', $article->id], ['confirm' => __('Are you sure you want to delete # {0}?', $article->id), 'class' => 'btn btn-danger btn-sm mt-1', 'escape' => false]) ?> 
<?= $this->Form->create ?>
<?php if ($article->published == 1) {
	echo $this->Form->postLink(__('<i class="fas fa-exclamation-triangle"></i> Unpublish'), ['action' => 'articlePublished', $article->id, $article->published], ['block' => true, 'confirm' => __('Are you sure you want to unpublish # {0}?', $article->title), 'class' => 'btn btn-danger btn-sm mt-1', 'escape' => false, 'title'=>'Unpublish Article']);
}else
	echo $this->Form->postLink(__('<i class="fas fa-check"></i> Publish'), ['action' => 'articlePublished', $article->id, $article->published], ['block' => true, 'confirm' => __('Are you sure you want to publish # {0}?', $article->title), 'class' => 'btn btn-success btn-sm mt-1', 'escape' => false, 'title'=>'Publish Article']); ?>

<?php if ($article->featured == 1) {
	echo $this->Form->postLink(__('<i class="fas fa-exclamation-triangle"></i> Unfeatured'), ['action' => 'articleFeatured', $article->id, $article->featured], ['block' => true, 'confirm' => __('Are you sure you want to unfeatured # {0}?', $article->title), 'class' => 'btn btn-danger btn-sm mt-1', 'escape' => false, 'title'=>'Unfeatured Article']);
}else
	echo $this->Form->postLink(__('<i class="fas fa-check"></i> Featured'), ['action' => 'articleFeatured', $article->id, $article->featured], ['block' => true, 'confirm' => __('Are you sure you want to featured # {0}?', $article->title), 'class' => 'btn btn-success btn-sm mt-1', 'escape' => false, 'title'=>'Featured Article']); ?>
...
<div class="card border-primary mb-3">
  <div class="card-header module-blue">
		<div class="icon-robot icon-robot-chat mt-0 text-light">Popular Reading</div>
  </div>
